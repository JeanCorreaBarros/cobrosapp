...
export default function BuscadorPrestamo({
  valor,
  onSeleccionar,
}: {
  valor: Prestamo | null;
  onSeleccionar: (prestamo: Prestamo | null) => void;
}) {
  const [texto, setTexto] = useState("");
  const [resultados, setResultados] = useState<Prestamo[]>([]);
  ...
  function buscar(valorTexto: string) {
    ...
    debounce.current = setTimeout(async () => {
      setCargando(true);
      const respuesta = await fetch(`/api/prestamos?buscar=${encodeURIComponent(valorTexto)}`);
      const datos = await respuesta.json();
      setResultados(datos.prestamos ?? []);
      setCargando(false);
    }, 250);
  }
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-borde bg-lienzo/70 px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-texto-3">
            {moneda(Number(valor.montoTotal))} total
          </p>
        </div>
            ...
            resultados.map((prestamo) => (
              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-xs text-texto-3">
                    {moneda(Number(prestamo.montoTotal))} total
                  </p>
                </div>
                <Pildora tono={TONO_ESTADO[prestamo.estado]}>{prestamo.estado}</Pildora>
              </button>
            ))
          )}
        </div>
